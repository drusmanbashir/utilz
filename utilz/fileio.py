...
def save_file_wrapper(fnc):
    ...
    def _inner(object, filename, verbose=False, overwrite=True, makedir=True):
        ...
        if overwrite == True or not filename.exists():
            ...
            print(
                "File {0} exists. Set overwrite to True if necessary".format(filename)
            )
...
def save_sitk(
    ...
    if not parent_folder.exists():
        maybe_makedirs(parent_folder)
    writer = sitk.ImageFileWriter()
    writer.SetFileName(output_filename)
    writer.Execute(img)
    if verbose == True:
        print("Saved to file: {}".format(output_filename))
...
def rename_and_move_images_and_masks(
    project_title,
    img_files,
    mask_files=None,
    counter_start=0,
